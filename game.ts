const gameGrid = document.querySelector(".gameGrid") as HTMLDivElement
const rollDicePlayer1 = document.querySelector("#rollDicePlayer1") as HTMLButtonElement;
const rollDicePlayer2 = document.querySelector("#rollDicePlayer2") as HTMLButtonElement;
const buyProperty1 = document.querySelector("#buyProperty1") as HTMLButtonElement;
const buyProperty2 = document.querySelector("#buyProperty2") as HTMLButtonElement;
...
let monopolyBoard: monopolyInterface[] = [
...
let playerBalances: number[] = [1500, 1500];
let playerProperties: number[][] = [[], []];
...
function updateBoard() {
    ...
    arr.map(item => {
        ...
        const colorClass = property?.color ? property.color.replace(/\s+/g, "-").toLowerCase() : '';
        const chanceImage = property?.type === "special" && property?.name === "Chance"
            ? '<img src="https://i.pinimg.com/originals/77/8d/b2/778db27d1a38f11413a0146fec674702.gif" alt="" class="chanceImg"/>'
            : '';
        const luxuryTax = property?.type === "tax" && property?.name === "Luxury Tax"
            ? '<img src="https://monopolyleeds.com/wp-content/uploads/2024/04/Ring.png" alt="" class="luxuryTax"/>'
            : '';
        const waterWorks = property?.name === "Water Works"
            ? '<img src="https://clipart-library.com/img1/994821.gif" alt="Water Works" class="waterWorks"/>'
            : '';
        const railroadImage = property?.type === "railroad" &&
        (property?.name === "B&O Railroad" || property?.name === "Short Line" || property?.name === "Reading Railroad")
            ...
            : '';
        const goImage = property?.name === "GO"
            ...
            : '';
        const freeParking = property?.name === "Free Parking"
        ...
        const incomeTax = property?.type === "tax" && property?.name === "Income Tax"
            ...
            : '';
        const chest = property?.type === "special" && property?.name === "Community Chest"
...
function updateMoneyDisplay() {
    const moneyPlayer1 = document.getElementById('moneyPlayer1') as HTMLSpanElement;
    if (moneyPlayer1) {
        moneyPlayer1.textContent = `Money: $${playerBalances[0]}`;
    }

    const moneyPlayer2 = document.getElementById('moneyPlayer2') as HTMLSpanElement;
    if (moneyPlayer2) {
        moneyPlayer2.textContent = `Money: $${playerBalances[1]}`;
    }
}
function calculateRent(property: monopolyInterface, playerIndex: number): number {
...
function updateInfobox(message: string): void {
    const infobox = document.querySelector('.infobox') as HTMLDivElement;
    const logMessage = document.createElement('p');
    logMessage.textContent = message;
    infobox.appendChild(logMessage);
    infobox.scrollTop = infobox.scrollHeight;
}
function collectMoney(playerIndex: number): void {
    playerBalances[playerIndex] += 200;
    updateInfobox(`Player ${playerIndex + 1} collected $200.`);
}

function buyProperty(playerIndex: number): void {
...
function updatePlayerCards(): void {
    const propertyPlayer1 = document.getElementById('propertyPlayer1') as HTMLSpanElement;
    const propertyPlayer2 = document.getElementById('propertyPlayer2') as HTMLSpanElement;
